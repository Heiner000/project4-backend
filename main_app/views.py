...
def user_portfolio_values(request):
    if request.method == "GET":
        try:
            ...
            total_portfolio_gain_loss = round(((total_portfolio_value - total_cost_basis) / total_cost_basis) * 100, 2) if total_cost_basis != 0 else Decimal(0)

            print('total portfolio gain/loss: ', total_portfolio_gain_loss)
            ...
            return JsonResponse(
                {
                    ...
                    "total_portfolio_gain_loss": str(total_portfolio_gain_loss)
                },
                safe=False,
            )
...
def follow_user(request):
    ...
    if follower_id is None or followed_id is None:
        return JsonResponse(
            {"error": "Both follower_id and followed_id are required."}, status=400
        )
